...
import { getPythonSetting, pathExistsSync, readFileSync, shellExecute } from '../externalDependencies';
...
export class Poetry {
    ...
    public static async getPoetry(cwd: string): Promise<Poetry | undefined> {
        ...
        logVerbose(`Getting poetry for cwd ${cwd}`);
    ...
    private static async locate(cwd: string): Promise<Poetry | undefined> {
        ...
        for (const poetryPath of getCandidates()) {
            logVerbose(`Probing poetry binary for ${cwd}: ${poetryPath}`);
            const poetry = new Poetry(poetryPath, cwd);
            const virtualenvs = await poetry.getEnvList();
            if (virtualenvs !== undefined) {
                logVerbose(`Found poetry via filesystem probing for ${cwd}: ${poetryPath}`);
                return poetry;
            }
        }

        // Didn't find anything.
        logVerbose(`No poetry binary found for ${cwd}`);
        return undefined;
    }
    ...
    private async getEnvListCached(_cwd: string): Promise<string[] | undefined> {
        ...
        return result.stdout.splitLines().map((line: any) => {
    ...
    private async safeShellExecute(command: string, fLogVerbose = false) {
        ...
        logVerbose(`Time taken to run ${command} in ms`, stopWatch.elapsedTime);
        return result;
    }
}
