        ...
        async function* iterator() {
            ...
            const envGenerators = envRootDirs.map((envRootDir) => {
                async function* generator() {
                    logVerbose(`Searching for custom virtual envs in: ${envRootDir}`);
                    ...
                    for await (const entry of executables) {
                        ...
                        if (await looksLikeBasicVirtualPython(entry)) {
                            try {
                                ...
                                logVerbose(`Custom Virtual Environment: [added] ${filename}`);
                            } catch (ex) {
                                logError(`Failed to process environment: ${filename}`, ex);
                            }
                        } else {
                            logVerbose(`Custom Virtual Environment: [skipped] ${filename}`);
                        }
                    }
                }
                return generator();
