        ...
        async function* iterator(root: string) {
            ...
            const envGenerators = envRootDirs.map((envRootDir) => {
                async function* generator() {
                    logVerbose(`Searching for workspace virtual envs in: ${envRootDir}`);
                    ...
                    for await (const entry of executables) {
                        ...
                        if (await looksLikeBasicVirtualPython(entry)) {
                            ...
                            logVerbose(`Workspace Virtual Environment: [added] ${filename}`);
                        } else {
                            logVerbose(`Workspace Virtual Environment: [skipped] ${filename}`);
                        }
                    }
                }
                return generator();
