...
import { arePathsSame, getWorkspaceFolders, isParentPath } from "../../../../pythonEnvironments/common/externalDependencies";
import { Uri } from "vscode";
import { parseVersionFromExecutable } from "../../info/executable";
import { logError, logWarning } from "../../../../common/logging";
import { getRegistryInterpreters, getRegistryInterpretersSync } from "../../../../pythonEnvironments/common/windowsUtils";
import { buildEnvInfo, comparePythonVersionSpecificity, getEnvDisplayString, getEnvMatcher } from "../../info/env";
import { getEnvironmentDirFromPath, getInterpreterPathFromDir, getPythonVersionFromPath } from "../../../../pythonEnvironments/common/commonUtils";
import { AnacondaCompanyName, Conda } from "../../../../pythonEnvironments/common/environmentManagers/conda";
import { parsePyenvVersion } from "../../../../pythonEnvironments/common/environmentManagers/pyenv";
...
async function resolveCondaEnv(executablePath: string): Promise<PythonEnvInfo> {
    ...
    if (conda === undefined) {
        logWarning(`${executablePath} identified as Conda environment even though Conda is not installed`);
    }
...
async function updateEnvUsingRegistry(env: PythonEnvInfo): Promise<void> {
    ...
    if (!interpreters) {
        logError('Expected registry interpreter cache to be initialized already');
    ...
    if (data) {
        ...
        logWarning('Expected registry to find the interpreter as source was set');
    }
}