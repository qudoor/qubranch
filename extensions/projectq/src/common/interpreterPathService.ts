...
export const defaultInterpreterPathSetting: keyof IProjectQSettings = 'defaultInterpreterPath';
...
export class InterpreterPathService implements IInterpreterPathService {
    ...
    public inspect(resource: Resource): InspectInterpreterSettingType {
        resource = ProjectQSettings.getSettingsUriAndTarget(resource, this.workspaceService).uri;
    ...
    public async update(
        ...
        if (!resource) {
            logError('Cannot update workspace settings as no workspace is opened');
            return;
        }
