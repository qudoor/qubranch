...
import { IDisposable, IOutputChannel, Resource } from '../types';
...
export class BackupPipInstaller {
    constructor(
        ...
        private readonly isInstalled: (packageName: string, interpreter: PythonEnvironment) => Promise<boolean | undefined>
    ) { }
    public async install(
        ...
        const isPipInstalled = await this.isInstalled("pip", interpreter);
        ...
        try {
            ...
            const options: ProgressOptions = {
                ...
                cancellable: true,
                title: DataScience.installingModule().format(packageName)
            };
            ...
            const isInstalled = await this.isInstalled(packageName, interpreter);
            traceVerbose(
                `After successfully running pip install product is ${isInstalled ? '' : 'still not'
            ...
            traceError(`Failed to Pip install ${packageName} into ${getDisplayPath(interpreter.path)}`, ex);
            return false;
        }
    }
    private async installImplmentation(
        ...
        const args = this.getInstallerArgs({ packageName, reinstall: reInstallAndUpdate });
        const cwd = resource ? this.workspaceService.getWorkspaceFolder(resource)?.uri.fsPath : undefined;
        this.outputChannel.appendLine(`Pip Installing ${packageName} into ${getDisplayPath(interpreter.path)}`);
        ...
        try {
            ...
            traceVerbose(`Successfully ran pip installer for ${packageName}`);
            return true;
        } finally {
    ...
    private getInstallerArgs(options: { reinstall: boolean; packageName: string }): string[] {
        const args: string[] = [];
        ...
        args.push('--user');
        return [...args, options.packageName];
    }
}
