...
import { IConfigurationService, IInstaller, InstallerResponse, IOutputChannel, IPersistentStateFactory, Product, ProductInstallStatus, Resource } from '../types';
import { IInstallationChannelManager, IModuleInstaller, InterpreterUri, IProductPathService, IProductService, ModuleInstallFlags, ProductType } from './types';
...
export async function trackPackageInstalledIntoInterpreter(
    ...
    const key = `${getInterpreterHash(interpreter)}#${packageName}`;
...
export class DataScienceInstaller extends BaseInstaller {
    ...
    public async install(
        ...
        const version = `${interpreter.version?.major || ''}.${interpreter.version?.minor || ''}.${interpreter.version?.patch || ''
            }`;
