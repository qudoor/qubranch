    ...
    public async showQuickPick<T extends QuickPickItem, P extends IQuickPickParameters<T>>({
        ...
        try {
            return await new Promise<MultiStepInputQuickPicResponseType<T, P>>((resolve, reject) => {
                const input: any = this.shell.createQuickPick<T>();
                input.title = title;
                input.step = step;
                ...
                disposables.push(
                    input.onDidTriggerButton(async (item: any) => {
                    ...
                    input.onDidChangeSelection((selectedItems: any) => resolve(selectedItems[0])),
                    input.onDidHide(() => {
                        resolve(undefined);
                    }),
                );
