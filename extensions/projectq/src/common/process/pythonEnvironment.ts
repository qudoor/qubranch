...
class PythonEnvironment {
    ...
    public async getModuleVersion(moduleName: string): Promise<string | undefined> {
        ...
        try {
            ...
            logInfo(`Error when getting version of module ${moduleName}`, ex);
            return undefined;
        }
    ...
    public async isModuleInstalled(moduleName: string): Promise<boolean> {
        ...
        try {
            ...
            logInfo(`Error when checking if module is installed ${moduleName}`, ex);
            return false;
        }
        return true;
    }
    ...
    private async getInterpreterInformationImpl(): Promise<InterpreterInformation | undefined> {
        try {
            const python = this.getExecutionInfo();
            return await getInterpreterInfo(python, this.deps.shellExec, { info: logInfo, error: logError });
        } catch (ex) {
            logError(`Failed to get interpreter information for '${this.pythonPath}'`, ex);
        }
    }
}
