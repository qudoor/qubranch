...
import { sendTelemetryPythonEvent as sendTelemetryEvent } from '../../telemetry';
...
function sendOptInOptOutTelemetry(optedIn: string[], optedOut: string[], packageJson: Record<string, unknown>): void {
    const optedInEnumValues = readEnumValues('projectq.experiments.optInto', packageJson);
    const optedOutEnumValues = readEnumValues('projectq.experiments.optOutFrom', packageJson);
