...
class ProjectQOutputChannelTransport extends Transport {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
// Create a ProjectQ output channel targeting transport that can be added to a winston logger.
export function getProjectQOutputChannelTransport(channel: OutputChannel, formatter: logform.Format) {
    return new ProjectQOutputChannelTransport(channel, {
        // We minimize customization.
        format: formatter
    });
}