...
import { TraceOptions, } from '../../common/logging/trace';
import { logVerbose, logWarning, logError, logInfo, traceDecorators } from '../../common/logging';
...
export class EnvironmentActivationService implements IEnvironmentActivationService, IDisposable {
    ...
    public async getActivatedEnvironmentVariablesImpl(
        ...
        try {
            ...
            logVerbose(`Activation Commands received ${activationCommands} for shell ${shellInfo.shell}`);
            ...
            logVerbose(`${hasCustomEnvVars ? 'Has' : 'No'} Custom Env Vars`);
            ...
            logVerbose(`Activating Environment to capture Environment variables, ${command}`);
            ...
            while (!result) {
                try {
                    ...
                    if (result.stderr) {
                        if (returnedEnv) {
                            logWarning('Got env variables but with errors', result.stderr);
                        } else {
                    ...
                    if (condaRetryMessages.find((m) => excString.includes(m)) && tryCount < 10) {
                        logInfo(`Conda is busy, attempting to retry ...`);
                        result = undefined;
                        tryCount += 1;
                        await sleep(500);
                    } else {
            ...
            logError('getActivatedEnvironmentVariables', e);
    ...
    @traceDecorators.error(
        'Failed to parse Environment variables',
    )
    @traceDecorators.verbose(
        'parseEnvironmentOutput', TraceOptions.None
    )
