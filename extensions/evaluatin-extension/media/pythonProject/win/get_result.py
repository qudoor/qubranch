...
def get_cpu_info():
    ...
    for _cpu in c.Win32_Processor():
        ...
        cpu_info['CPU型号'] = is_None(_cpu.Name.strip())
        cpu_info['主频'] = '%sMHz' % is_None(_cpu.MaxClockSpeed)
        cpu_info['核心数'] = is_None(_cpu.NumberOfCores)
        cpu_info['线程数'] = is_None(_cpu.ThreadCount)
        cpu_info['L1 Cache'] = is_None(int(
            _cpu.NumberOfCores) * 32 if _cpu.NumberOfCores else '0')
        cpu_info['L2 Cache'] = is_None(_cpu.L2CacheSize)
        cpu_info['指令集'] = ''
...
if __name__ =='__main__':
    '''
    python3 get_result.py --tag 1652757977 --job myjobname
    '''
