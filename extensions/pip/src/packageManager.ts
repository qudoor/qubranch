...
export const necessaryPackage = ["pip", "setuptools", "wheel"];
...
export class PackageManager implements IPackageManager {
  ...
  constructor(private _pythonPath: string, @IOutputChannel private readonly output: IOutputChannel, @IExtensionContext private readonly context: IExtensionContext) {
  ...
  private pip(args: string[], cancelToken?: vscode.CancellationToken, showErrorMessage = true) {
    ...
    return this.execute(python, ["-m", "pip"].concat(args).concat([]), cancelToken).catch(err => {
      if (showErrorMessage) {
  ...
  public async getPackageVersionList(pack: string | PackageInfo, cancelToken?: vscode.CancellationToken) {
    ...
    try {
      ...
      versionList = (versions || "")
        .replace(/\s+/g, "")
        .split(",")
        .filter(version => version && version !== "none")
        .reverse();
    }
